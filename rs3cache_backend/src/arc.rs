...
impl Archive {
    ...
    pub(crate) fn deserialize_jag(metadata: &Metadata, mut buffer: Bytes) -> CacheResult<Archive> {
        ...
        let decompressed_len = BufExtra::try_get_uint(buffer, 3).context(error::Read { what: "metadata" })?;
        let compressed_len = BufExtra::try_get_uint(buffer, 3).context(error::Read { what: "metadata" })?;
