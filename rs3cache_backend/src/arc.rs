...
impl Archive {
    ...
    pub(crate) fn deserialize_jag(metadata: &Metadata, mut buffer: Bytes) -> CacheResult<Archive> {
        ...
        let decompressed_len = BufExtra::try_get_uint(&mut buffer, 3).context(error::Read { what: "metadata" })?;
        let compressed_len = BufExtra::try_get_uint(&mut buffer, 3).context(error::Read { what: "metadata" })?;
        ...
        for i in 0..files_length {
            let header = JagHeader {
                filename: BufExtra::try_get_i32(&mut headers).context(error::Read { what: "metadata" })?,
